...
## Adding [React Redux](https://react-redux.js.org/)

Redux is essential in a Vite React app for managing global state efficiently, providing a predictable state container that helps with debugging and maintaining consistent application behavior. It centralizes state management, making it easier to scale and handle complex state interactions across the app.

Following the [offical](https://react-redux.js.org/introduction/getting-started) documentations

### [Installation](https://react-redux.js.org/tutorials/quick-start)
...
In your project, create a new folder called store inside the src directory.
Inside the store folder, create a file named store.ts.
Add the following code to store.ts to set up the Redux store:
...
<details>
<summary>Slice Explanation</summary>
A slice in Redux Toolkit is a way to define a portion of your Redux state and the reducers that operate on that state in a single file. It simplifies the process of creating and managing Redux state by bundling the state, action creators, and reducers together.

### Key Components of a Slice:
- State: Defines the initial state for the slice. It's a piece of the overall Redux state tree that this slice will manage.

- Reducers: Functions that handle state changes based on actions. In a slice, these reducers are defined as functions inside the createSlice call and automatically generate corresponding action creators.

- Actions: Generated automatically by createSlice for each reducer. These are the action creators you'll use to dispatch actions to the store.

- Name: A unique identifier for the slice, used to namespace the actions and state.
...
This defines a simple state with one property, value, initialized to 0.
...
```
Here, three reducers are defined: increment, decrement, and incrementByAmount. Each reducer modifies the state based on the action dispatched.
...
These are the action creators automatically generated by createSlice. They can be dispatched to the store to invoke the corresponding reducer.
...
This exports the reducer function created by createSlice so it can be included in the store.

</details>


Inside the store folder, create a new file named exampleSlice.ts.
Add the following code to exampleSlice.ts to create a slice:
...
4. Add the Slice to the Store

Open the store.ts file.

Import the reducer from your slice and add it to the store configuration.

Update store.ts to look like this:
...
5. Provide the Redux Store to Your React App

Open your main.tsx file (or index.tsx if that's what you have).

Wrap your <App /> component with the <Provider /> component from react-redux and pass in the store.

Update main.tsx to look like this:
...
6. Use Redux State and Actions in Your Components

In a component, use the useSelector hook to access the state and the useDispatch hook to dispatch actions.

We will try on the home component by adding a button to increase and decrease the number.

1. First lets display the content of the example state's value
...
3. adding onclick handleres to update the state final code look like below
